        ...
        <div class="bg-white p-6 rounded-lg shadow-lg">
            ...
            <table class="min-w-full bg-white rounded-lg shadow-md">
                ...
                <tbody class="text-gray-700">
                    @foreach ($sppt as $index => $Sppt)
                    <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}">
                        <td class="w-1/12 py-3 px-4 text-sm">{{ $index + 1 }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Sppt->user->nama }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Sppt->user->nik }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Sppt->user->no_wa }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Sppt->user->alamat }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">
                            <button class="bg-gray-800 text-white py-1 px-2 rounded hover:bg-blue-600 transition duration-200" onclick="showModal('{{ Storage::url($Sppt->ktp) }}', '{{ Storage::url($Sppt->kk) }}','{{$Sppt->batas_utara}}','{{$Sppt->batas_selatan}}','{{$Sppt->batas_timur}}','{{$Sppt->batas_barat}}')">Lihat Dokumen</button>
                        </td>
                        <td class="w-1/12 py-3 px-4 text-sm">{{ $Sppt->status }}</td>
                        <td class="w-1/12 py-3 px-4 text-sm">
                            <a href="{{ route('SPPT.show', $Sppt->id) }}" class="bg-gray-800 text-white py-1 px-2 rounded hover:bg-blue-600 transition duration-200">Show</a>
                            <a href="{{ route('SPPT.edit', $Sppt->id) }}" class="bg-gray-800 text-white py-1 px-2 rounded hover:bg-blue-600 transition duration-200">Edit</a>
                            <form action="{{ route('SPPT.destroy', $Sppt->id) }}" method="POST" class="inline">
