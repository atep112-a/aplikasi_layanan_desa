...
<body class="bg-gray-100">
    <div class="container mx-auto mt-8">
        <div class="bg-white p-6 rounded-lg shadow-lg">
            <h2 class="text-3xl font-bold mb-6 text-gray-900">Daftar Domisili</h2>
            <table class="min-w-full bg-white rounded-lg shadow-md">
                ...
                <tbody class="text-gray-700">
                    @foreach ($domisili as $index => $Domisli)
                    <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }}">
                        <td class="w-1/12 py-3 px-4 text-sm">{{ $index + 1 }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Domisli->nama }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Domisli->nik }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Domisli->no_wa }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">{{ $Domisli->alamat }}</td>
                        <td class="w-2/12 py-3 px-4 text-sm">
                            <button class="bg-gray-800 text-white py-1 px-2 rounded hover:bg-blue-600 transition duration-200" onclick="showModal('{{ Storage::url($Domisli->ktp) }}')">Lihat Dokumen</button>
                        </td>
                        <td class="w-1/12 py-3 px-4 text-sm">{{ $Domisli->status }}</td>
                        <td class="w-1/12 py-3 px-4 text-sm">
                            <a href="{{ route('domisili.edit', $Domisli->id) }}" class="bg-gray-800 text-white py-1 px-2 rounded hover:bg-blue-600 transition duration-200">Edit</a>
                            <form action="{{ route('domisili.destroy', $Domisli->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
